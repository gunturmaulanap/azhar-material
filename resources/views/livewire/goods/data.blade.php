<div x-data="{ open: false, openBrand: false }" @keydown.escape.window="open=false; openBrand=false">
    ...
    @php
        // otomatis hasilkan prefix rute sesuai role: super_admin -> superadmin, dll
    ...
    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center z-50"
        ...
        <div
            ...
            <button class="absolute right-2 top-2 rounded-full bg-white" type="button" @click="open = false">
                <svg xmlns="http://www.w3.org/2000/svg" class="size-8 text-red-500" fill="none" viewBox="0 0 24 24"
                                    ...
                                    <tr class="border-b transition-colors hover:bg-gray-50">
                                        ...
                                        <td class="py-2">
                                            <div class="flex items-center gap-x-4 justify-center">
                                                ...
                                                <button wire:click="validationDelete({{ $category->id }}, 'category')"
    ...
    <div x-cloak x-show="openBrand" class="fixed inset-0 flex items-center justify-center z-50"
        ...
        <div
            class="relative bg-white rounded-lg overflow-hidden shadow-xl transform transition-all sm:w-full sm:max-w-4xl">
            <button class="absolute right-2 top-2 rounded-full bg-white" type="button" @click="openBrand = false">
                                    ...
                                    <tr class="border-b transition-colors hover:bg-gray-50">
                                        ...
                                        <td class="py-2">
                                            <div class="flex items-center gap-x-4 justify-center">
                                                ...
                                                <button wire:click="validationDelete({{ $brand->id }}, 'brand')"
                                                    class="px-2 py-1 flex items-center gap-x-2 rounded-md bg-red-500 text-xs text-white">
                                                    Hapus
                                                </button>
                                            </div>
            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    @forelse ($data as $item)
                        <tr class="border-b transition-colors hover:bg-gray-50" wire:key="goods-{{ $item->id }}">
                ...
                <div class="flex w-full flex-col gap-1">
                    <div class="flex items-center">
                        <div class="flex flex-col items-start gap-2 w-2/3">
                            ...
                            <div class="line-clamp-2 text-xs text-muted-foreground">
                                ...
                                <div class="grid grid-cols-3">
                                    ...
                                    <div class="col-span-2">: {{ optional($item->brand)->name ?? 'Tidak ada merk' }}
                                    </div>
                                </div>
                                @if (auth()->user()->role === 'super_admin')
                                    <div class="grid grid-cols-3">
                        ...
                        <div class="flex flex-col ml-auto text-sm text-end">
                            <span><span class="font-extrabold">{{ $item->stock }}</span> {{ $item->unit }}</span>

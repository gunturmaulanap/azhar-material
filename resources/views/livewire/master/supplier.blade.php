<div>
    <x-slot name="title">{{ __('Data Supplier') }}</x-slot>

    <x-slot name="breadcrumb">
        @php
            $breadcumb = ['Data', 'Supplier'];
        @endphp
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('superadmin.master.create-supplier') }}"
                ...
                <thead>
                    <tr class="border-b">
                        <th class="h-10 px-2 text-left">
                            <span
                                class="whitespace-nowrap inline-flex font-medium items-center justify-center px-3 text-sm -ml-3">
                                Nama Perusahaan
                            </span>
                        </th>
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr class="border-b transition-colors hover:bg-gray-50">
                            ...
                            <td class="p-2 max-w-52 sm:max-w-24 xl:max-w-52">
                            ...
                            <td class="py-2">
                                <div class="flex items-center gap-x-4 justify-center">
                                    <a href="{{ route('superadmin.master.update-supplier', ['id' => $item->id]) }}"
        ...
        @foreach ($data as $item)
            <button class="flex flex-col items-start gap-2 rounded-lg border p-3 text-left text-sm w-full">
                ...
                <div class="flex items-center gap-2 w-full justify-end mt-1">
                    ...
                    <a type="button" wire:click="validationDelete({{ $item->id }})"
                        class="px-2 py-1 flex items-center gap-x-2 rounded-md bg-red-500 text-xs text-white">

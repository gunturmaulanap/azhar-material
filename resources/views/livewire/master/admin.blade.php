<div>
    <x-slot name="title">{{ __('Data Admin') }}</x-slot>

    <x-slot name="breadcrumb">
        @php
            $breadcumb = ['Data', 'Admin'];
        @endphp
    ...
    <div class="flex items-center justify-between mb-4 gap-x-2">
        ...
        <a href="{{ route(str_replace('_', '', auth()->user()->role) . '.master.create-admin') }}"
            class="inline-flex items-center gap-x-2 px-2 py-1.5 text-xs bg-sky-500 text-white font-extrabold rounded-md shadow-md whitespace-nowrap">
    ...
    {{-- Main Table --}}
    <div class="rounded-md border bg-white whitespace-nowrap hidden sm:block">
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr class="border-b transition-colors hover:bg-gray-50">
                            <td class="p-2 px-4 w-[20%] whitespace-nowrap">
                                {{ $item->name }}
                            </td>
                            <td class="p-2 w-[30%]">
                                {{ $item->username }}
                            ...
                            <td class="py-2">
                                <div class="flex items-center gap-x-4 justify-center">
                                    <a href="{{ route(str_replace('_', '', auth()->user()->role) . '.master.update-admin', ['id' => $item->id]) }}"
        ...
        @foreach ($data as $item)
            <button class="flex flex-col items-start gap-2 rounded-lg border p-3 text-left text-sm w-full">
                <div class="flex w-full flex-col gap-1">
                    ...
                    <div class="text-xs font-medium">{{ $item->username }}</div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (isMobile()) {
            ...
            Livewire.emit('perpage', 4); // Atur perPage menjadi 5 untuk mobile
        } else {
